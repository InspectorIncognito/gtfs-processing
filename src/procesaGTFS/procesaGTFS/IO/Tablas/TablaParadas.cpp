...
void TablaParadas::Crear()
{
	...
	ofstream fileout;
	fileout.open(string(fdd_->parametros->carpetaOutput + "/" + fdd_->parametros->version + "/" + "ServerStops.csv").c_str());
...
//			continue;
		...
		ConvertCoordinate::UTMtoLL(23, (*ired).second.y, (*ired).second.x, fdd_->UTMZone, lat, lon);
