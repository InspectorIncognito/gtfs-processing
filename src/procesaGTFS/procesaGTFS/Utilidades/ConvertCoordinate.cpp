...
namespace ConvertCoordinate {
	...
	void UTMtoLL(int ReferenceEllipsoid, const double UTMNorthing, const double UTMEasting, string UTMZone,
		...
		double &UTMNorthing, double &UTMEasting, string& UTMZone)
	{
		...
		//std::sprintf(UTMZone, "%d%c", ZoneNumber, UTMLetterDesignator(Lat));
		//UTMZone.append()
		UTMZone = to_string(ZoneNumber);

		std::string s;
		std::stringstream ss;
		ss << UTMLetterDesignator(Lat);
		ss >> s;
